 ...
 * @date   7 April 2015
 * @version 0.1
 * @brief   An introductory character driver to support the second article of my
 * series on Linux loadable kernel module (LKM) development. This module maps to
 * /dev/ebbchar and comes with a helper C program that can be run in Linux user
 * space to communicate with this the LKM.
...
#define DEVICE_NAME                                                            \
  "ebbchar" ///< The device will appear at /dev/ebbchar using this value
#define CLASS_NAME                                                             \
  "ebb" ///< The device class -- this is a character device driver

MODULE_LICENSE(
    "GPL"); ///< The license type -- this affects available functionality
MODULE_AUTHOR("Derek Molloy"); ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION(
    "A simple Linux char driver for the BBB"); ///< The description -- see
                                               ///< modinfo
...
static int __init ebbchar_init(void) {
  ...
  ebbcharDevice = device_create(ebbcharClass, NULL, MKDEV(majorNumber, 0), NULL,
                                DEVICE_NAME);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len,
  ...
  if (bytesNotCopied) {
    printk(KERN_INFO "EBBChar: Failed to send % d characters to he user\n ", bytesNotCopied);
    return -EFAULT;
  }
  size_of_message = 0;
  return bytesToCopy;
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len,
  ...
  size_t bytesToCopy = len >= maxLen
                           ? maxLen
                           : len; // If len is 255 or more, copy only 255 bytes
  size_t bytesNotCopied = 0;
  /* copy_from_user returns 0 if successful,
  else it returns the number of bytes *NOT* copied */
  bytesNotCopied = copy_from_user(message, buffer, bytesToCopy);
  sprintf(message + bytesToCopy - bytesNotCopied, " (%zu letters)",
          bytesToCopy - bytesNotCopied);
  size_of_message =
      bytesToCopy - bytesNotCopied + 1; // Include the NULL-character
...
// static ssize_t dev_write(struct file *filep, const char *buffer, size_t len,
//                         loff_t *offset) {
//  sprintf(message, "%s(%zu letters)", buffer,
